...
fn leakage_objective_swaps_to_high_vt_when_slack_allows() {
    ...
    assert!(r.after_leak_w < r.before_leak_w, "leakage should drop: {} -> {}", r.before_leak_w, r.after_leak_w);
    assert_eq!(r.changed.len(), 2, "both inverters move to HVT");
    assert!(r.changed.iter().all(|(_, old, new)| old == "INV" && new == "INV_HVT"));
...
fn leakage_objective_keeps_critical_cells_low_vt() {
    ...
    assert!(r.changed.is_empty(), "no slack -> keep low-Vt, no leakage swap");
...
fn timing_objective_speeds_up_with_low_vt() {
    ...
    assert!(r.before_wns < 0.0, "HVT should violate at 0.28 ns: {}", r.before_wns);
    assert!(r.after_wns > r.before_wns, "low-Vt swap should improve setup");
    assert!(r.changed.iter().any(|(_, _, new)| new == "INV"), "expected a swap to fast INV");
}
